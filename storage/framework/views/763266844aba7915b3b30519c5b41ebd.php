...
<form action="<?php echo e(isset($editUser) ? route('abalos-users.update', $editUser->id) : route('abalos-users.store')); ?>" method="POST" class="bg-white p-4 rounded shadow-md mb-6">
    ...
    <div class="mb-4">
        ...
        <input type="password" id="password" name="password" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring focus:ring-blue-500" placeholder="<?php echo e(isset($editUser) ? 'Leave blank to keep the current password' : 'Enter password'); ?>" <?php echo e(isset($editUser) ? '' : 'required'); ?>>
    </div>
    <div class="mb-4">
    ...
    <select id="role" name="role" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring focus:ring-blue-500">
        <option value="" disabled <?php echo e(isset($editUser) && $editUser->role == '' ? 'selected' : ''); ?>>Select user role</option>
        <option value="admin" <?php echo e(isset($editUser) && $editUser->role == 'admin' ? 'selected' : ''); ?>>Admin</option>
        <option value="user" <?php echo e(isset($editUser) && $editUser->role == 'user' ? 'selected' : ''); ?>>User</option>
    </select>
</div>
<div class="mb-4">
    ...
    <select name="status" id="status" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring focus:ring-blue-500">
        <option value="active" <?php echo e(isset($editUser) && $editUser->status == 'active' ? 'selected' : ''); ?>>Active</option>
        <option value="inactive" <?php echo e(isset($editUser) && $editUser->status == 'inactive' ? 'selected' : ''); ?>>Inactive</option>
            ...
            <tbody>
                <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="hover:bg-gray-100">
                        ...
                        <td class="py-2 px-4 border-b"><?php echo e($user->password); ?></td>
                        <td class="py-2 px-4 border-b"><?php echo e($user->role); ?></td>
                        <td class="py-2 px-4 border-b"><?php echo e($user->status); ?></td>
...
<?php endif; ?>
<?php /**PATH C:\Users\abalo\midact3\resources\views/abaloschristianuser.blade.php ENDPATH**/ ?>