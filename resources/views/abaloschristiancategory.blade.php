...
<body class="bg-gray-100">
    <nav class="bg-blue-500 p-4 flex items-center">
        ...
        <div class="ml-8 flex space-x-4">
            ...
            <a href="{{ route('abalos-categories.index') }}" class="text-white flex items-center hover:underline active">
    ...
    <div class="container mx-auto p-6">
        ...
        <h2 class="text-xl font-semibold mb-2">{{ isset($editCategory) ? 'Edit Category' : 'Create Category' }}</h2>
        <form action="{{ isset($editCategory) ? route('abalos-categories.update', $editCategory->id) : route('abalos-categories.store') }}" method="POST" class="bg-white p-4 rounded shadow-md mb-6">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" id="name" name="name" value="{{ isset($editCategory) ? $editCategory->name : '' }}" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring focus:ring-blue-500" placeholder="Enter category name">
            </div>
        ...
        <!-- Search Bar -->
        <div class="mb-4">
            <form action="{{ route('abalos-categories.index') }}" method="GET" class="flex items-center">
                            ...
                            <tr class="border-b">
                                ...
                                <td class="py-2 px-4">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('abalos-categories.edit', $category->id) }}" class="text-blue-500 hover:underline">Edit</a>
                                        <form action="{{ route('abalos-categories.destroy', $category->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
